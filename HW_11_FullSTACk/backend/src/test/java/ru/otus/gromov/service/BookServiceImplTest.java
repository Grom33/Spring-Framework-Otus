...
public class BookServiceImplTest {
    ...
    @Before
    public void setUp() {
        service.save(TEST_BOOK_1).block();
        service.save(TEST_BOOK_2).block();
        service.save(TEST_BOOK_3).block();
    }

    @Test
    public void count() {
        Assert.assertEquals("Count of Books", 3L, service.count().block().longValue());
    }
    ...
    public void save() {
        Book testBook = service.save(new Book("TestBook4",
                ...
                new LinkedHashSet<>(Collections.singletonList(TEST_AUTHOR1)))).block();
        Assert.assertEquals("Insert Book to base",
                testBook,
                service.getById(testBook.getId()).block());
        Assert.assertEquals("Count entity in base", 4, service.count().block().longValue());
    }
    ...
    public void getById() {
        Assert.assertEquals("Get Book by ID",
                TEST_BOOK_1,
                service.getById(TEST_BOOK_1.getId()).block());
    }
    ...
    public void getByName() {
        Assert.assertEquals("Get Author by Name",
                TEST_BOOK_1,
                service.getByName(TEST_BOOK_1.getName()).block());
    }
    ...
    public void delete() {
        ...
        service.delete(TEST_BOOK_1.getId()).block();
