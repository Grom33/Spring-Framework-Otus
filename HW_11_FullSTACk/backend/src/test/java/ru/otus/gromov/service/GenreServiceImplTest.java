...
public class GenreServiceImplTest {
    ...
    @Before
    public void setUp() {
        service.save(TEST_GENRE_1).block();
        service.save(TEST_GENRE_2).block();
        service.save(TEST_GENRE_3).block();
    }

    @Test
    public void count() {
        Assert.assertEquals("Get count of Genre", 3, service.count().block().longValue());
    }

    @Test
    public void save() {
        Genre testGenre = service.save(new Genre("test4")).block();
        Assert.assertEquals("Save Genre",
                testGenre,
                service.getById(testGenre.getId()).block());
        Assert.assertEquals("Save Genre, check count",
                4,
                service.count().block().longValue());
    }
    ...
    public void getById() {
        Assert.assertEquals("Save Genre",
                TEST_GENRE_1,
                service.getById(TEST_GENRE_1.getId()).block());
    }
    ...
    public void getByName() {
        Assert.assertEquals("Save Genre",
                TEST_GENRE_1,
                service.getByName(TEST_GENRE_1.getName()).block());
    }
    ...
    public void delete() {
        ...
        service.delete(TEST_GENRE_1.getId()).block();
