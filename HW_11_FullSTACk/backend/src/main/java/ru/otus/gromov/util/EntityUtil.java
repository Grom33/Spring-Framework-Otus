...
public class EntityUtil {
    ...
    private static Set<Author> arrayToSetAuthor(String[] array, AuthorService authorService) {
        Set<Author> result = new HashSet<>();
        Arrays.stream(array).forEach(i -> result.add(authorService.getById(i).block()));
        return result;
    }

    private static Set<Genre> arrayToSetGenre(String[] array, GenreService genreService) {
        Set<Genre> result = new HashSet<>();
        Arrays.stream(array).forEach(i -> result.add(genreService.getById(i).block()));
        return result;
    }
